...
5. 浏览器开 **http://localhost:9000**(由 `.env` 的 `HOST_PORT` 控制)。
...
./deploy.sh            # 构建并启动
```
要求 Docker 宿主内核支持 CIFS(多数发行版自带 `cifs` 模块)。WebUI:http://<服务器IP>:9000。

## 三、直接跑在 NAS / 本地盘(文件在本机,用绑定挂载)

适合把 Mikannet 部署在 **NAS 自身的 Docker**(Synology/QNAP 等)或文件就在本地的 Linux。
...
数据(SQLite、封面缓存、日志)落在 `./data/mikannet`,迁移机器把它一起带走即可。

## 端口 / 卷一览

- **9000** → WebUI(改宿主端口:设置 `.env` 的 `HOST_PORT`)；容器内仍为 8008。
- `downloads` → 下载根(容器内 `/downloads`);`nas_import` → 旧番剧导入(`/import-nas`);`bd_owned` → 已购原盘(`/bd-owned`);`./data/mikannet` → 运行时数据(`/config`)。
