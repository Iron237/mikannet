...
class ParsedTitle:
    ...
    ep_type: str = "regular"
    # 先行(抢先/先行配信版):标题带「先行」标记。发布日兜底(早于开播日)由 rss_engine 补判。
    ...
    def to_dict(self) -> dict:
        return {"episodes": self.episodes, "version": self.version, "is_batch": self.is_batch,
                ...
                "ep_type": self.ep_type, "is_preview": self.is_preview,
    ...
    def from_dict(cls, d: dict) -> "ParsedTitle":
        return cls(episodes=d.get("episodes") or [], version=d.get("version", 1),
                   ...
                   ep_type=d.get("ep_type", "regular"), is_preview=d.get("is_preview", False),
...
# 先行(抢先/先行配信版):标题带「先行」即判(涵盖 先行版/先行配信/先行上映/abema先行版)。
# 正片是官方开播后的正常发布(无此标记),两者分流去重、分开归档。
_PREVIEW = re.compile(r"先行|アドバンス先行", re.I)
...
def _parse_rules(title: str) -> ParsedTitle:
    ...
    result = ParsedTitle(resolution=_detect_resolution(t), version=_detect_version(t),
                         ...
                         is_preview=bool(_PREVIEW.search(t)))
