"""首次配置向导:存储(本地/SMB/NFS)→ 下载器/代理/元数据(复用 /api/config)→ 完成。
...
def test_storage(payload: dict):
    ...
    return storage.test(mode, (payload.get("smb_host_path") or "").strip(),
                        ...
                        (payload.get("nfs_host_path") or "").strip(),
                        (payload.get("nfs_options") or "").strip())
...
def save_storage(payload: dict):
    ...
    if mode not in ("smb", "nfs", "local"):
        raise HTTPException(400, "mode 必须是 local、smb 或 nfs")
    fields = {
        "storage_mode": mode,
        "local_host_path": storage.normalize_windows_path(payload.get("local_host_path") or ""),
    }
    if mode == "smb":
        fields["smb_host_path"] = storage.normalize_smb_path(payload.get("smb_host_path") or "")
        ...
        fields["nfs_host_path"] = (payload.get("nfs_host_path") or "").strip()
        fields["nfs_options"] = (
            payload.get("nfs_options") or "vers=4,soft,timeo=30,retrans=2").strip()
    storage.save(**fields)
    result = storage.apply()
    if mode in ("smb", "nfs") and not result.get("mounted"):
