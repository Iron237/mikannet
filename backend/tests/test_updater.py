...
def test_check_with_only_latest_stable_release(monkeypatch):
    """清理旧 Release 后，客户端不依赖基线版本仍可直升唯一的新版本。"""
    _patch(monkeypatch, base_rev_remote="baseAAA")
    latest = dict(_releases()[-1], prerelease=False)
    monkeypatch.setattr(updater, "_list_releases", lambda: [latest])

    r = updater.check(include_prerelease=False)
...
def test_safe_extract_rejects_sibling_prefix_escape(tmp_path):
    """`dest/../dest-evil` 不能因字符串前缀相同而绕过目录边界。"""
    blob = _make_targz({"../extract-evil/payload.py": "# escaped\n"})
    with pytest.raises(RuntimeError, match="非法路径"):
        updater._safe_extract(blob, tmp_path / "extract")
    assert not (tmp_path / "extract-evil" / "payload.py").exists()
