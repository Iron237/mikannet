...
- 当前版本未内置账户登录；只应部署在可信局域网或 VPN 内，**不要直接暴露到公网**。
- `data/`、备份 JSON、`.env`、Cookie 和通知/第三方令牌均含隐私信息，不得提交或作为 Issue 附件上传。
- 发布前运行 `python scripts/check_public_repo.py`、后端测试和前端构建；完整清单见 [RELEASING.md](RELEASING.md)。
